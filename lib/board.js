...
class Board {
  ...
  set(row, col, color, options = {}) {
    if (this.checkLegal(row, col, color) || options.force) {
      this._board[row][col] = color;
      const other = util.oppositeColor(color);
      this.turn = this.canPlay(other) ? other : color;
    }
  }
  ...
  legalMovesFor(color = this.turn) {
    ...
    for (let i = 0; i < boardSize; i ++) {
      for (let j = 0; j < boardSize; j ++) {
        if (this.checkLegal(i, j, color)) { result.push([i, j]); }
      }
    }
    return result;
  }

  canPlay(color = this.turn) {
    for (let i = 0; i < boardSize; i ++) {
      for (let j = 0; j < boardSize; j ++) {
        if (this.checkLegal(i, j, color)) { return true; }
      }
    }
    return false;
  }
  ...
  isGameOver() {
    return this.canPlay(white) || this.canPlay(black);
  }
